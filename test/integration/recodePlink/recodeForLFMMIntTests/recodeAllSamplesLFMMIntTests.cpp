...
SCENARIO("Test that RecodeLFMM recodes data correctly",
         "[recode-lfmm-data-int]") {
    ...
    GIVEN("An input file to split")
    {
	    ...
	    std::string fileNameExpectedOutput(pathToInputFolder + "expected-recoded-data-lfmm-all-samples.txt");

	    int nbIndiv(786), nbLignes(nbIndiv), nbSnp(5), nbColonnesRecodees(5);
