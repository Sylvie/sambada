...
SCENARIO("Test that RecodePlink recodes data correctly when a subset of samples is selected",
         "[recode-plink-data-subset-int]") {
    ...
    GIVEN("An input file to split")
    {
	    ...
	    std::string fileNameExpectedOutput(pathToInputFolder + "expected-recoded-data-subset-samples.txt");

	    int nbIndiv(786), nbIndivSelectionnes(136), nbLignes(nbIndivSelectionnes+1), nbSnp(5), nbColonnesRecodees(16);
