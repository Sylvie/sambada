...
SCENARIO("Test that Storey's p-values histograms are correct with score threshold for models of dimension 1",
        "[storey-histograms-dim-1-int][storey-histograms-threshold-int]") {
    ...
    GIVEN("A program to run, its expected output file, and the file of expected results")
    {
        ...
        std::string pathToTestFolder("test/integration/sambada/storeyHistogramsWithScoreThreshold/univariateModels/");
