...
SCENARIO("Test that Storey's p-values histograms are correct with population structure as first variables",
		"[storey-histograms-with-pop-structure-as-first-variables-int][storey-histograms-int]") {
    ...
    GIVEN("A program to run, its expected output file, and the file of expected results")
    {
        ...
        std::string fileNameParam(pathToInputFolder + "param-with-pop-structure-as-first-variables.txt");
	    ...
	    std::string fileNameExpectedResults(pathToInputFolder + "expected-storey-histograms-with-pop-structure-as-first-variables.txt");
