...
SCENARIO("Test that geographically weighted regression is computed correctly for spherical coordinates and \"distance\" weighting ",
         "[gwr-spherical-distance-int][gwr-int]")
{
	...
	GIVEN("Some input files with info on population structure")
	{
		...
		std::string fileNameExpectedResultsDim1(pathToInputFolder + "expected-results-spherical-distance-dim-1.txt");
		std::string fileNameExpectedResultsDim2(pathToInputFolder + "expected-results-spherical-distance-dim-2.txt");
