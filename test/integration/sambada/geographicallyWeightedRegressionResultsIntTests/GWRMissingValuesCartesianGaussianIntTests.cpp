...
SCENARIO("Test that geographically weighted regression is computed correctly for cartesian coordinates and \"gaussian\" weighting with some missing environmental values",
         "[gwr-missing-values-cartesian-gaussian-int][gwr-int]")
{
	...
	GIVEN("Some input files with info on population structure")
	{
		...
		std::string fileNameOut2(pathToOutputFolder + "random-sample-Out-2.txt");
		...
		std::string fileNameParams(pathToInputFolder + "param-cartesian-gaussian.txt");
		std::string fileNameData(pathToInputFolder + "random-sample-missing-values.txt");

		std::string fileNameExpectedResultsDim0(pathToInputFolder + "expected-results-random-sample-dim-0.txt");
		std::string fileNameExpectedResultsDim1(pathToInputFolder + "expected-results-missing-values-cartesian-gaussian-dim-1.txt");
		std::string fileNameExpectedResultsDim2(pathToInputFolder + "expected-results-missing-values-cartesian-gaussian-dim-2.txt");
		...
		expectedNullResults.verifieTailles(true, 0, 1, false, true);
		...
		lecteurCorrige.close();
		expectedResultsDim1.verifieTailles(true, 1, 2, false, true);
