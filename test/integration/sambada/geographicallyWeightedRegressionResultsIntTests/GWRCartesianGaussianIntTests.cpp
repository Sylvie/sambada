...
SCENARIO("Test that geographically weighted regression is computed correctly for cartesian coordinates and \"gaussian\" weighting ",
         "[gwr-cartesian-gaussian-int][gwr-int]")
{
	...
	GIVEN("Some input files with info on population structure")
	{
		...
		std::string fileNameExpectedResultsDim1(pathToInputFolder + "expected-results-cartesian-gaussian-dim-1.txt");
		std::string fileNameExpectedResultsDim2(pathToInputFolder + "expected-results-cartesian-gaussian-dim-2.txt");
