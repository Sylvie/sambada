...
SCENARIO("Test that geographically weighted regression is computed correctly for cartesian coordinates and \"distance\" weighting ",
         "[gwr-cartesian-distance-int][gwr-int]")
{
	...
	GIVEN("Some input files with geolocated samples")
	{
		...
		std::string fileNameExpectedResultsDim1(pathToInputFolder + "expected-results-cartesian-distance-dim-1.txt");
		std::string fileNameExpectedResultsDim2(pathToInputFolder + "expected-results-cartesian-distance-dim-2.txt");
