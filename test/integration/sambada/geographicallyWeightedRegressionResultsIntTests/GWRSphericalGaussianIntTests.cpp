...
SCENARIO("Test that geographically weighted regression is computed correctly for spherical coordinates and \"gaussian\" weighting ",
         "[gwr-spherical-gaussian-int][gwr-int]")
{
	...
	GIVEN("Some input files with info on population structure")
	{
		...
		std::string fileNameParams(pathToInputFolder + "param-spherical-gaussian.txt");
		...
		std::string fileNameExpectedResultsDim1(pathToInputFolder + "expected-results-spherical-gaussian-dim-1.txt");
		std::string fileNameExpectedResultsDim2(pathToInputFolder + "expected-results-spherical-gaussian-dim-2.txt");
