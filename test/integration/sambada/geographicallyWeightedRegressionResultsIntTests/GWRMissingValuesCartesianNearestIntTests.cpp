...
SCENARIO("Test that geographically weighted regression is computed correctly for cartesian coordinates and \"nearest\" weighting with some missing environmental values",
         "[gwr-missing-values-cartesian-nearest-int][gwr-int]")
{
	...
	GIVEN("Some input files with info on population structure")
	{
		...
		std::string fileNameParams(pathToInputFolder + "param-cartesian-nearest.txt");
		...
		std::string fileNameExpectedResultsDim1(pathToInputFolder + "expected-results-missing-values-cartesian-nearest-dim-1.txt");
		std::string fileNameExpectedResultsDim2(pathToInputFolder + "expected-results-missing-values-cartesian-nearest-dim-2.txt");
