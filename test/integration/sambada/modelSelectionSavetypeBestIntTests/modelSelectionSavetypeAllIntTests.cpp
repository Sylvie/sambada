...
SCENARIO("Test that significant models are correctly selected with option \"SAVETYPE ALL\"",
    ...
    GIVEN("Some input files with info on population structure")
    {
        ...
        std::string fileNameParams(pathToInputFolder + "param-savetype-all.txt");
        std::string fileNameEnv(pathToInputFolder + "cattle-pop-env.csv");
        std::string fileNameMark(pathToInputFolder + "cattle-pop-mark.txt");

        std::string fileNameExpectedResultsDim0(pathToInputFolder + "expected-results-savetype-all-dim-0.txt");
        std::string fileNameExpectedResultsDim1(pathToInputFolder + "expected-results-savetype-all-dim-1.txt");
        std::string fileNameExpectedResultsDim2(pathToInputFolder + "expected-results-savetype-all-dim-2.txt");
        std::string fileNameExpectedResultsDim3(pathToInputFolder + "expected-results-savetype-all-dim-3.txt");
