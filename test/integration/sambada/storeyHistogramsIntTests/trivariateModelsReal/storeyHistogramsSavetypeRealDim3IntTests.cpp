...
SCENARIO("Test that Storey's p-values histograms are correct with \"SAVETYPE REAL\" for models of dimension 3",
		"[storey-histograms-no-threshold-savetype-real-dim-3-int][storey-histograms-int]") {
    ...
    GIVEN("A program to run, its expected output file, and the file of expected results")
    {
        ...
        std::string pathToTestFolder("test/integration/sambada/storeyHistogramsIntTests/trivariateModelsReal/");
