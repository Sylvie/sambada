...
SCENARIO("Test that Storey's p-values histograms are correct with score threshold, population structure as first variables, and \"SAVETYPE END\"",
		"[storey-histograms-with-score-threshold-and-pop-structure-as-first-variables-savetype-end-int][storey-histograms-threshold-int]") {
    ...
    GIVEN("A program to run, its expected output file, and the file of expected results")
    {
        ...
        std::string pathToTestFolder("test/integration/sambada/storeyHistogramsScoreThresholdIntTests/trivariateModelsEnd/");
