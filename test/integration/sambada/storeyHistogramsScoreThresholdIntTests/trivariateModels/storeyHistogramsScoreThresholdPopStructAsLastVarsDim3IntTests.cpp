...
SCENARIO("Test that Storey's p-values histograms are correct with score threshold, \"SAVETYPE REAL\" and population structure as last variables for trivariate models",
		"[storey-histograms-score-threshold-savetype-real-pop-structure-as-last-variables-int][storey-histograms-threshold-int]") {
    ...
    GIVEN("A program to run, its expected output file, and the file of expected results")
    {
        ...
        std::string fileNameExpectedResultsDim1(pathToInputFolder + "expected-results-pop-last-dim-1.txt");
