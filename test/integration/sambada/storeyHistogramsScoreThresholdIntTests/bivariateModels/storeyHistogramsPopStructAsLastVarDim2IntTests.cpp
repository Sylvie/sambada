...
SCENARIO("Test that Storey's p-values histograms are correct without score threshold and population structure as last variables for bivariate models",
		"[storey-histograms-savetype-real-pop-structure-as-last-variables-dim-2-int][storey-histograms-threshold-int]") {
    ...
    GIVEN("A program to run, its expected output file, and the file of expected results")
    {
        ...
        std::string fileNameParam(pathToInputFolder + "param-with-pop-structure-as-last-variables-no-threshold-dim-2.txt");
	    ...
	    std::string fileNameExpectedResults(pathToInputFolder + "expected-storey-histograms-with-pop-structure-as-last-variables-dim-2.txt");
        ...
        std::string fileNameExpectedResultsDim2(pathToInputFolder + "expected-results-bi-no-threshold-pop-last-dim-2.txt");
