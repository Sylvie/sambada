...
SCENARIO("Test that regression results are correct with \"SAVETYPE REAL\" when the population structure is placed after the environmental variables",
         "[pop-struct-as-last-var-real-int][pop-struct-int]") {
    ...
    GIVEN("Some input files with info on population structure")
    {
        ...
        std::string pathToTestFolder("./test/integration/sambada/basicPopulationStructureIntTests/trivariateModelsReal/");
        ...
        std::string fileNameExpectedResultsDim2(pathToInputFolder + "expected-results-pop-last-dim-2.txt");
