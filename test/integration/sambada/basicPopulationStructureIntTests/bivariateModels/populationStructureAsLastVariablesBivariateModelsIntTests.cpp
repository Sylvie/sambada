...
SCENARIO("Test that regression results are correct when the population structure is placed after the environmental variables for bivariate models",
         "[pop-struct-as-last-var-bi-int][pop-struct-int]") {
    ...
    GIVEN("Some input files with info on population structure")
    {
        ...
        std::string fileNameParams(pathToInputFolder + "param-bi-pop-last.txt");
	    ...
	    std::string fileNameExpectedResultsDim2(pathToInputFolder + "expected-results-bi-pop-last-dim-2.txt");
