...
SCENARIO("Test that regression results are correct when the filenames have no extensions",
         "[no-extensions-filenames-int]") {
    ...
    GIVEN("Some input files with or without extensions to their names")
    {
        ...
        std::string fileNameOut0NoExt(pathToOutputFolder + "choice-mark-cattle-Out-0");

        std::string fileNameOut1(pathToOutputFolder + "choice-mark-cattle-Out-1.txt");
        std::string fileNameOut1NoExt(pathToOutputFolder + "choice-mark-cattle-Out-1");
        ...
        std::string fileNameMarkNoExt(pathToInputFolder + "choice-mark-cattle");
