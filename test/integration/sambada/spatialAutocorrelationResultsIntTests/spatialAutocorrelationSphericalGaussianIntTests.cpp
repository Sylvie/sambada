...
SCENARIO("Test that spatial autocorrelation is computed correctly for spherical coordinates and gaussian weighting",
         "[spatial-autocorrelation-spherical-gaussian-int]") {
    ...
    GIVEN("Some input files with sampling locations")
    {
        ...
        std::string fileNameParams(pathToInputFolder + "param-random-sample-spherical-gaussian.txt");
	    std::string fileNameData(pathToInputFolder + "random-sample.txt");
	    int nombrePoints(100), nombreVariablesEnv(2), nombreMark(1);

        std::string fileNameExpectedResultsASEnv(pathToInputFolder + "expected-results-spherical-gaussian-env.txt");
        std::string fileNameExpectedResultsASEnvPVal(pathToInputFolder + "expected-results-spherical-gaussian-env-pval.txt");
        std::string fileNameExpectedResultsASMark(pathToInputFolder + "expected-results-spherical-gaussian-mark.txt");
        std::string fileNameExpectedResultsASMarkPVal(pathToInputFolder + "expected-results-spherical-gaussian-mark-pval.txt");
