...
SCENARIO("Test that spatial autocorrelation is computed correctly for cartesian coordinates and gaussian weighting",
         "[spatial-autocorrelation-cartesian-gaussian-int]") {
    ...
    GIVEN("Some input files with sampling locations")
    {
        ...
        std::string fileNameParams(pathToInputFolder + "param-random-sample-cartesian-gaussian.txt");
	    std::string fileNameData(pathToInputFolder + "random-sample.txt");
	    int nombrePoints(100), nombreVariablesEnv(2), nombreMark(1);

        std::string fileNameExpectedResultsASEnv(pathToInputFolder + "expected-results-cartesian-gaussian-env.txt");
        std::string fileNameExpectedResultsASEnvPVal(pathToInputFolder + "expected-results-cartesian-gaussian-env-pval.txt");
        std::string fileNameExpectedResultsASMark(pathToInputFolder + "expected-results-cartesian-gaussian-mark.txt");
        std::string fileNameExpectedResultsASMarkPVal(pathToInputFolder + "expected-results-cartesian-gaussian-mark-pval.txt");
