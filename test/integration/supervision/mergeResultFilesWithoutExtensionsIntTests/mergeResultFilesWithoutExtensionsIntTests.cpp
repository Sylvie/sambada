...
SCENARIO("Test that Supervision merges results without filename extensions correctly for trivariate models",
    ...
    GIVEN("Some result files from Sambada")
    {
        ...
        std::string pathToTestFolder("test/integration/supervision/mergeResultFilesWithoutExtensionsIntTests/");

        std::string pathToOutputFolder("./" + pathToTestFolder);

        std::string fileNameOut0(pathToOutputFolder + "mol-data-res-0");
        std::string fileNameOut1(pathToOutputFolder + "mol-data-res-1");
        std::string fileNameOut2(pathToOutputFolder + "mol-data-res-2");
        std::string fileNameOut3(pathToOutputFolder + "mol-data-res-3");
        ...
        std::string fileNameMark(pathToOutputFolder + "mol-data");
        ...
        for (int i(0); i <= dimensionMax; ++i)
        {
            for (int j(0); j < numBlocs; ++j)
            {
                ...
                std::to_string(j*tailleBlocs) + "-Out-" + std::to_string(i));
