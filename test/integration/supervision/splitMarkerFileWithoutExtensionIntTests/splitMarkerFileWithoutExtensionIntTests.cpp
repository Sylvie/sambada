...
SCENARIO("Test that Supervision splits input files correctly when they have no filename extensions",
         "[split-marker-file-no-ext-int]") {
    ...
    GIVEN("An input file to split")
    {
	    ...
	    std::string pathToTestFolder("test/integration/supervision/splitMarkerFileWithoutExtensionIntTests/");

	    std::string pathToOutputFolder("./" + pathToTestFolder);

        std::string fileNameOutEnv(pathToOutputFolder + "mol-data-env");
        std::string fileNameOutMark0(pathToOutputFolder + "mol-data-mark-0-0");
        std::string fileNameOutMark1(pathToOutputFolder + "mol-data-mark-1-12");
	    std::string fileNameOutMark2(pathToOutputFolder + "mol-data-mark-2-24");
        ...
        std::string fileNameMark(pathToInputFolder + "mol-data");

	    std::string fileNameParamInOutputFolder(pathToOutputFolder + bareFileNameParams);
	    std::string fileNameMarkInOutputFolder(pathToOutputFolder + "mol-data");
