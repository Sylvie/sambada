...
SCENARIO("Test that Supervision merges results correctly for trivariate models with a custom word delimitor",
         "[merge-result-files-custom-delim-int]") {
    ...
    GIVEN("Some result files from Sambada")
    {
        ...
        std::string pathToTestFolder("test/integration/supervision/mergeResultFilesWithCustomDelimitorIntTests/");
	    ...
	    std::string scoreThresholdType = "G";
	    int scoreThreshold(0);
	    std::string sortScore("Wald"), delimitor("\"|\"");
        ...
        std::string fileNameExpectedResultsDim2(pathToInputFolder + "expected-results-dim-2.txt");
        std::string fileNameExpectedResultsDim3(pathToInputFolder + "expected-results-dim-3.txt");
        ...
        WHEN("Supervision is run on the split result files")
        {
            ...
            THEN("the output files are found")
            {
                ...
                {
                    SambadaInputData resultsDim0(
                            SambadaIntegrationTestUtils::readInputData(lecteurOut0, 1));
	                SambadaInputData resultsDim1(
                            SambadaIntegrationTestUtils::readInputData(lecteurOut1, 1));
	                SambadaInputData resultsDim2(
                            SambadaIntegrationTestUtils::readInputData(lecteurOut2, 1));
	                SambadaInputData resultsDim3(
                            SambadaIntegrationTestUtils::readInputData(lecteurOut3, 1));

                    THEN("the results match the expectations")
                    {
                        INFO("Verifying results dim 0");
                        resultsDim0.verifieTailles(31, 1);
                        resultsDim0.compare(expectedNullResults);

                        INFO("Verifying results dim 1");
                        resultsDim1.verifieTailles(211, 1);
                        resultsDim1.compare(expectedResultsDim1);

                        INFO("Verifying results dim 2");
                        resultsDim2.verifieTailles(631, 1);
                        resultsDim2.compare(expectedResultsDim2);

                        INFO("Verifying results dim 3");
                        resultsDim3.verifieTailles(1051, 1);
